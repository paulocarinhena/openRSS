...
// Mensagens do zod são chaves de settings.errors.
...
export async function updateSettingsAction(input: z.input<typeof schema>) {
  ...
  // Mantém o cookie alinhado para as páginas públicas (login) após sair.
  if (data.uiLanguage) await writeLocaleCookie(data.uiLanguage);
...
export async function updateProfileAction(name: string) {
  ...
  if (!value.success) return { ok: false as const, error: (await getTranslations("settings.errors"))("invalidName") };
